...
class CatalogScreen(DirectFrame):
    ...
    def setClarabelleChat(self, stringy, timeout = 6):
        ...
        chatTextNode.setWordwrap(CHAT_TEXT_WORD_WRAP)
        chatTextNode.setGlyphScale(TEXT_GLYPH_SCALE)
        chatTextNode.setGlyphShift(TEXT_GLYPH_SHIFT)
        chatTextNode.setText(stringy)
        chatTextNode.setFont(ToontownGlobals.getInterfaceFont())

        self.clarabelleChat = ChatBalloon(
            self.clarabelleChatBalloon , 10, 1, chatTextNode)

        self.clarabelleChatNP = self.attachNewNode(self.clarabelleChat.node(), 500)
        self.clarabelleChatNP.setScale(0.08, 1, 0.08)
