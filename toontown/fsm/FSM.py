...
class FSM(DirectObject):
    ...
    self.state_ may be queried at any time other than during the
    handling of the enter() and exit() functions.  During these
    functions, self.state_ contains the value None (you are not really
    ...
    via self.oldState and self.newState.  At other times, self.state_
    ...
    def getCurrentFilter(self):
        ...
        filter = getattr(self, "filter" + self.state_, None)
        if not filter:
    ...
    def demand(self, request, *args):
        ...
        try:
            ...
            if not self.request(request, *args):
                raise RequestDenied("%s (from state: %s)" % (request, self.state_))
        finally:
            self.fsmLock.release()
    ...
    def defaultFilter(self, request, args):
        ...
        if self.defaultTransitions is None:
            ...
            # indicated state.
            if request in self.defaultTransitions.get(self.state_, []):
        ...
        # (lowercase) requests.
        assert self.notify.debug("%s ignoring request %s from state %s." % (self._name, request, self.state_))
        return None
    ...
    def __callEnterFunc(self, name, *args):
        ...
        assert self.state_ == None and self.newState == name
    ...
    def __callFromToFunc(self, oldState, newState, *args):
        ...
        # a new state, if it exists.
        assert self.state_ == None and self.oldState == oldState and self.newState == newState
    ...
    def __callExitFunc(self, name):
        ...
        # state, if it exists.
        assert self.state_ == None and self.oldState == name
    ...
    def __str__(self):
        ...
        try:
            ...
            if self.state_:
                str = ('%s FSM:%s in state "%s"' % (className, self._name, self.state_))
            else:
