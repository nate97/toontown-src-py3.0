...
class DistributedPoliceChipAI(DistributedCCharBaseAI.DistributedCCharBaseAI):
    ...
    def _DistributedPoliceChipAI__decideNextState(self, doneStatus):
    ...
    def enterLonely(self):
        self.lonely.enter()
        self.acceptOnce(self.lonelyDoneEvent, self._DistributedPoliceChipAI__decideNextState)
        if self.dale:
    ...
    def _DistributedPoliceChipAI__goForAWalk(self, task):
    ...
    def enterChatty(self):
        self.chatty.enter()
        self.acceptOnce(self.chattyDoneEvent, self._DistributedPoliceChipAI__decideNextState)
        if self.dale:
            self.dale.chipEnteringState(self.fsm.getCurrentState().getName())
    ...
    def enterWalk(self):
        self.notify.debug('going for a walk')
        self.walk.enter()
        self.acceptOnce(self.walkDoneEvent, self._DistributedPoliceChipAI__decideNextState)
        if self.dale:
            self.dale.chipEnteringState(self.fsm.getCurrentState().getName())
