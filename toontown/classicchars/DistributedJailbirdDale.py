...
class DistributedJailbirdDale(DistributedCCharBase.DistributedCCharBase):
    notify = DirectNotifyGlobal.directNotify.newCategory('DistributedJailbirdDale')

    def __init__(self, cr):
        try:
            self.DistributedJailbirdDale_initialized
        except:
            self.DistributedJailbirdDale_initialized = 1
    ...
    def delete(self):
        try:
            self.DistributedJailbirdDale_deleted
        except:
            del self.fsm
            self.DistributedJailbirdDale_deleted = 1
